...
Adafruit_NeoPixel pixels = Adafruit_NeoPixel(1, 9, NEO_GRBW + NEO_KHZ800);
byte rgb[3]; // Declare RGB array
...
void status(byte inPage) { // Displays single color
  if ((millis() - effectMillis) > 10) {
    wheel(inPage*40);
    pixels.setPixelColor(0, pixels.Color(b*rgb[0]/255, b*rgb[1]/255, b*rgb[2]/255, 0));
    pixels.show();
    effectMillis = millis();
  }
}
...
void blinkLED() { // Blinks LEDs once
  pixels.setPixelColor(0, pixels.Color(b*100/255, b*100/255, b*100/255, 0));
  pixels.show();
  delay(50);
...
void wheel(byte shortColor) { // Set RGB color with byte
  ...
  if (shortColor >= 0 && shortColor < 85) {
    rgb[0] = (shortColor * -3) +255;
    rgb[1] = shortColor * 3;
    rgb[2] = 0;
  }
  if (shortColor >= 85 && shortColor < 170) {
    rgb[0] = 0;
    rgb[1] = ((shortColor - 85) * -3) +255;
    rgb[2] = (shortColor - 85) * 3;
  }
  if (shortColor >= 170 && shortColor < 255) {
    rgb[0] = (shortColor - 170) * 3;
    rgb[1] = 0;
    rgb[2] = ((shortColor - 170) * -3) +255;
  }
}
...
void sideButton(){
  ...
  if (hold == 2) {
    ...
    for (byte x = 0; x < numkeys; x++) {
      if (!bounce[x]) page = x; // Changes page depending on which key is pressed (1-6)
    }
  }
  if(bounce[numkeys]){
    ...
    for (int a = 0; a < numkeys; a++) { // checks each key
      if (!pressed[a]) { // This made sense to me at one point
        for (int b = 0; b < 3; b++) if (!bounce[a]) { // For each
          if (mapping[page][a][b] != 0) Keyboard.press(mapping[page][a][b]); // only run if real character is mapped
          pressed[a] = 1; // nonsense
        }
      }
      if (pressed[a]) {
        for (int b = 0; b < 3; b++) if (bounce[a]) {
          if (mapping[page][a][b] != 0) Keyboard.release(mapping[page][a][b]);
          pressed[a] = 0;
        }
      }
    }
